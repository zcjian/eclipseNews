...
<div id="main">
  ...
  <div id="opt_area">
     <h1 id="opt_type"> 修改栏目： </h1>
    <form action="admin/column?method=update" method="post">
      ...
      <p>
        ...
        <select name="parentId">
          ...
          <c:forEach items="${columns}" var="c">
           <option value='${c.id}' <c:if test="${column.parent.id==c.id}">selected="selected"</c:if>> ${c.name} </option>
         </c:forEach>
        </select>
      </p>
      <p>
        <label> 栏目名称 </label>
        <input name="name" type="text" class="opt_input" value="${column.name}" />
      </p>
